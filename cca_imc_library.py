...
# ------------------------------
# specific library (pip install)
try:
    from requests import get as getURL              # needed to extract things from Yahoo Finance website
except ImportError, e:
    raise Exception("CCA_LIB::REQUESTS may not have been installed properly, please try entering \"pip install requests\" in your terminal.")
try:
...
# --------------------------------------------------------------------------------------------------
# --------------------------------------------------------------------------------------------------
# Functions to retrieve a symbol from yahoo finance
def nFile(sym,yStart,yEnd):
    """Build a file name to store data about symbol SYM over specific range"""
...
def getSymbol(sym,yStart,yEnd):
    ...
    try:
        ...
        link = base+str(yEnd)+"&g=d&a=7&b=19&c="+str(yStart)+"&ignore=.csv&s="+sym.upper()
